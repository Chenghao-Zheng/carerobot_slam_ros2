 ...
 * @Description: ROS 2 Fast_Security header
...
private:
  ...
  rclcpp::Publisher<visualization_msgs::msg::MarkerArray>::SharedPtr corridorPub;
  // 连续部分系数
  // 连续部分距离势场系数
...
vector<Vector2i> &after1Extension,vector<Vector2i> &after2Extension);
  ...
  void visual_STATE_VISITEDNode(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pathPublish, 
                                std::vector<Eigen::Vector2d> visitnodes,
                                float a_set,float r_set,float g_set,float b_set,float length);

  // 可视化采样点集合
  void visual_SamplesNode(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr pathPublish, 
                          vector<vector<Vector2d>> samplenodes,
  ...
  ofstream file;
private:
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr visitNodesPub;  //访问节点发布节点
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr visitOptpathPub;  //优化的路径点发布
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr sampleNodesPub;    //采样点初始化
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr continuNodesPub;    //连续点初始化
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr suppleMentPub;     //补充发布
