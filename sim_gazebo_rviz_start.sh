#!/usr/bin/env bash
source ~/.bashrc
export GAZEBO_MODEL_DATABASE_URI=""          # 禁用网络模型数据库
export GAZEBO_IP="127.0.0.1"                 # 禁用外部网络连接
export GAZEBO_MODEL_PATH=/usr/share/gazebo-11/models:/home/ubuntu/carerobot_slam_ros1/src/diff_drive_gazebo/models   # 指定本地模型路径（根据实际情况调整）
...
gnome-terminal -t "DWA_control" -- bash -c \
"source ~/carerobot_slam_ros1/devel/setup.bash; 
roslaunch robot_navigation Differential_DWA.launch; exec bash"

# 9) 启动并发布机器人真值里程计消息
gnome-terminal -t "truth Odom start" -- bash -lc "source ~/carerobot_slam_ros1/devel/setup.bash;  roslaunch robot_locatization truth_odometry.launch;exec bash"
