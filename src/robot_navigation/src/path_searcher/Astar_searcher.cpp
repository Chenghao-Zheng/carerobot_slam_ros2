...
// 用于传参 (ROS 2 版)
void AstarPathFinder::InitParams(std::shared_ptr<rclcpp::Node> node, bool _is_use_esdf)
{
    // 只声明不存在的参数
    if (!node->has_parameter("astar_weight/g")) {
        node->declare_parameter<double>("astar_weight/g", 1.0);
    }
    if (!node->has_parameter("astar_weight/h")) {
        node->declare_parameter<double>("astar_weight/h", 1.0);
    }
    if (!node->has_parameter("astar_heuristic/distance")) {
        node->declare_parameter<int>("astar_heuristic/distance", 0);
    }
    if (!node->has_parameter("astar_glength/distance")) {
        node->declare_parameter<int>("astar_glength/distance", 0);
    }
    if (!node->has_parameter("astar_search/radius")) {
        node->declare_parameter<double>("astar_search/radius", 100.0);
    }

    node->get_parameter("astar_weight/g", g_value);
    node->get_parameter("astar_weight/h", h_value);
    node->get_parameter("astar_heuristic/distance", h_method);
    node->get_parameter("astar_glength/distance", g_method);
    node->get_parameter("astar_search/radius", search_radius);
...
{
    return (idx < X_SIZE && idy < Y_SIZE && data[idx * Y_SIZE + idy] >= 90);
}
    ...
    if (reach_horizon)
    {
		...
		if (is_use_esdf) 
		{
			...
			else if (map_cost > 0)
			{
				...
				double max_penalty = 200.0;  // 适中的惩罚
				penalty_cost = pow(cost_ratio, 2) * max_penalty;
			}
		}
