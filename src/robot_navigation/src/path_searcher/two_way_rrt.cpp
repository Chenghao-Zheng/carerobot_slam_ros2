                                        ...
                                        GetPlanMode mode)
{
    ...
    optimizationPath(path, this->angle_difference_);     //优化路径
...
void RRTstarPlanner::PublishPath(std::vector<Eigen::Vector2d> path)
{
    ...
    for(unsigned int i=0; i<path.size(); i++)
    {
        ...
        path_pose.header.stamp = rclcpp::Clock().now();
...
//发布整个树的可视化扩展
void RRTstarPlanner::pubTreeMarker(rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr &marker_pub, visualization_msgs::msg::Marker marker, int id)
{
...
bool RRTstarPlanner::FindPath(Eigen::Vector2d start, Eigen::Vector2d target)
{
    ...
    RCUTILS_LOG_WARN("failed to get a path.");
    return false;
}
